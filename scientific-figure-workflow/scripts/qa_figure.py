...
def run_qa(target: Path, plan: dict | None) -> dict:
    ...
    for path in files:
        if path.suffix.lower() == ".svg":
            checks.extend(inspect_svg(path))
        elif path.suffix.lower() == ".pdf":
            checks.extend(inspect_pdf(path))
        elif path.suffix.lower() == ".png":
            checks.extend(inspect_png(path))
    ...
    if plan:
        ...
        if has_data_panels:
            checks.append(check("data-render-source-present", "pass" if ".py" in suffixes else "fail"))
            provenance_files = [path for path in files if path.name in {"data-provenance.json", "provenance.json"}]
            checks.append(check("data-provenance-present", "pass" if provenance_files else "fail"))
            for path in provenance_files:
                checks.extend(verify_data_provenance(path))
    ...
    return {"schema_version": "1.1", "status": status, "target": str(target.resolve()), "checks": checks}
