...
- The Plugin also installs `$paperbanana` and `$autofigure-edit` wrapper Skills. Their pinned repositories and separate Python environments are prepared on demand with `scripts/figure.py backends install --backend all`; require explicit permission before the large dependency download. If absent, keep deterministic core routes available.
...
- Generated raster figures must use `annotation_spec.mode: deterministic-overlay`. The image model generates no text; `raster_annotation_backend.py` adds the reviewed title, key concepts, callouts, arrows, legend, and conceptual-evidence footer as an exact post-generation layer. Do not approve a raster plan until `visible_labels` and annotation positions communicate the figure without relying on the caption alone.
- Hybrid Figures must prove their raster/vector boundary from SVG source, not appearance. Required raster roles use `<image>` and must hash-match source assets; modules, arrows, text, bars, and axes use reviewed vector tags. Missing, mis-typed, unclassified, or stale-hash elements fail QA.
...
- `scripts/figure.py backends status` reports versioned external paths and readiness. Adapters automatically discover these runtimes. A full project checkout may use `scripts/install_team.ps1` for one-command Plugin, core runtime, and both-backend setup.
